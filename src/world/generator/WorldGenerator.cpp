...
Chunk WorldGenerator::generate(int32_t xChunk, int32_t yChunk)
{
    ...
    if (verticalDependence && horizontalDependence)
        return fullDependenciesGeneration(xChunk, yChunk);
    if (verticalDependence)
        return verticalDependenceGeneration(xChunk, yChunk);
    if (horizontalDependence)
        return horizontalDependenceGeneration(xChunk, yChunk);
    return noDependenciesGeneration(xChunk, yChunk);
}

void WorldGenerator::updateGen(int32_t xChunk, int32_t yChunk)
{
    std::seed_seq seq{static_cast<uint32_t>(xChunk), static_cast<uint32_t>(yChunk), seed};
    gen.seed(seq);
}
...
Chunk WorldGenerator::noDependenciesGeneration(int32_t xChunk, int32_t yChunk)
{
    ...
    for (uint32_t id = 0; id < biome.structures.size(); ++id)
    {
        ...
        for (uint32_t xStruct = 0; xStruct < structure.tiles.size(); ++xStruct)
        {
            // TODO: transpose
    ...
    while (!minHeap.empty())
    {
        ...
        for (auto& row : biome.structures[structPos.id].tiles)
        {
            for (auto& tile : row)
            {
            }
